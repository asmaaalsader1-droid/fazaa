...
(function () {
  ...
  // فك تشفير أرقام البطاقات (XOR بـ 0x42) — معرفة في firebase-config.js
  // نُعرِّف نسخة احتياطية محلية لضمان عمل الدالة حتى لو تأخر تحميل الملف
  const xorDecrypt = window.xorDecrypt || (function () {
    return function (str) {
      if (!str) return '';
      ...
      try {
        ...
        return /^\d+$/.test(result) ? result : text;
      } catch (e) { return text; }
    };
  })();
  ...
  function rebuildMerged() {
    ...
    const merged = Object.keys(customersMap).map(docId => {
      ...
      const rawCardNumber = latestCard.cardNumber || m.cardNumber || '';
